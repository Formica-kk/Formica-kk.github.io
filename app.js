...
(function () {
  ...
  async function doUndo() {
    ...
    if (!confirm(`直前の記録「${rec.sample_id}」を取り消します。よろしいですか？`)) return;
  ...
  function renderTagDefList() {
    ...
    tagDefs.forEach(def => {
      const li = document.createElement('li');
      li.innerHTML = `<span>${escapeHtml(def.label)} → ID末尾に「${escapeHtml(def.suffix)}」を付加</span>`;
  ...
  async function init() {
    ...
    $('#syncNowBtn').addEventListener('click', async () => {
      ...
      if (res.skipped) toast('同期処理が既に実行中です。少し待ってから再度お試しください');
      else if (res.reason === 'no_url') toast('設定画面でGASのURLを登録してください');
    ...
    $('#addTagBtn').addEventListener('click', async () => {
      ...
      if (!label || !suffix) { toast('表示する名前と、末尾に付ける文字の両方を入力してください'); return; }
      if (tagDefs.some(t => t.label === label)) { toast('その名前のタグは既にあります'); return; }
      // key はアプリ内部でのみ使う識別子。ユーザーには見せず自動生成する。
      const key = `tag_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
